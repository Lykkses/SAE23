...
class Avions(models.Model):
    id = models.IntegerField(blank = False)
    nom = models.CharField(max_lentgh=100)
...
class Vols(models.Model):
    ...
    date_darriver = models.DateField(null=True, blank=True)
    ...
    def __str__(self):
        chaine = f"Le vol numéro {self.id} avec l'avion {self.avions} avec le pilote {self.pilote}, décollera à {self.heure_de_depart} le {self.date_de_depart} à {self.aeroport_de_depart}. Le vol et arrivera à destination à {self.heure_de_darriver}, le {self.date_darriver}, à {self.aeroport_de_darriver}."
        return chaine

    def dico(self):
        return {"Le vol numéro":self.id, "avec l'avion":self.avions, "avec le pilote":self.pilote, ", décollera à ":self.heure_de_depart, "le":self.date_de_depart, "à":self.aeroport_de_depart, "Le vol arrivera à destination à":self.heure_de_darriver, "le":self.date_darriver, "à l'aéroport de":self.aeroport_de_darriver}
